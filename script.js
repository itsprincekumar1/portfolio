...
const config = {
    ...
    trailLength: 0.8,
    stopDecay: 0.85,
    // black / grey / silver palette for background + subtle metallic highlights
    color1: "#070708", // near black
    color2: "#2b2b2f", // dark grey
    color3: "#9aa0a6", // muted silver-grey
    color4: "#c8c8c8", // silver accent
    colorIntensity: 1.0,
    softness: 1.0,
};
...
const displatPlane = new THREE.Mesh(geometry,displayMaterial);
...
function animate() {
    ...
    // update time and frame uniforms correctly
    ...
    // update color uniforms (they were accidentally set to hex strings)
    const _c1 = hexToRgb(config.color1);
    const _c2 = hexToRgb(config.color2);
    const _c3 = hexToRgb(config.color3);
    const _c4 = hexToRgb(config.color4);
    displayMaterial.uniforms.uColor1.value.set(_c1[0], _c1[1], _c1[2]);
    displayMaterial.uniforms.uColor2.value.set(_c2[0], _c2[1], _c2[2]);
    displayMaterial.uniforms.uColor3.value.set(_c3[0], _c3[1], _c3[2]);
    displayMaterial.uniforms.uColor4.value.set(_c4[0], _c4[1], _c4[2]);
    ...
    renderer.render(displatPlane,camera);
